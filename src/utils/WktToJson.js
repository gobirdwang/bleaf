...
export function getMultiPolygon (MultiPolygon) {
  ...
  if (MultiPolygon.indexOf('MULTIPOLYGON(((') > -1 || MultiPolygon.indexOf('(((') > -1 && MultiPolygon.lastIndexOf(')))') > 0) {
    ...
    Result = new esri.geometry.Polygon(polygonJson)
  } else if (MultiPolygon.indexOf('POLYGON((') > -1 || (MultiPolygon.indexOf('((') > -1 && MultiPolygon.lastIndexOf('))') > 0)) {
    ...
    arrPoints = new Array()
    for (var q = 0; q < objPoints.length; q++) {
      objPoints[q] = objPoints[q].trim().split(' ')
      arrXy = new Array()
      arrXy.push(parseFloat(objPoints[q][0]))
      arrXy.push(parseFloat(objPoints[q][1]))
      arrPoints.push(arrXy)
    }
