...
## Kettle是什么？
Kettle是**水壶**。  
中国人生活中，开水绝对少不了的，原因当然是，未经处理的生水，含有各种杂质（脏数据），无法直接饮用（入库），为此，我们需要一个煮水的工具（ETL）。倒入（extract）水壶（Kettle）的水（数据），可能来自很多地方，如趵突泉（数据库）、黑虎泉（excel）、小清河（json）等，这些水只有烧开（transform）后,倒入（load）水杯中，才能供人享用。

当Kettle不再是水壶时，它是一个常用的**ETL（Extract-Transform-Load）工具**，负责接收抽取的数据，进行转换后，加载目的端。Kettle应用于以下场景：  
+ 不同应用或数据库之间整合数据；
+ 将数据库中的文件导出为文本文件；
+ 大批量数据装载进数据库；
+ 数据清洗；
